...
compile = function(){
  ...
  // @todo - different way of checking the right directory
  // try{
  //   config = JSON.parse(fs.readFileSync('./res/config.json', 'utf8',function(error){}));
  // }catch(e){
  //   console.log('This is probably not a resource directory!');
  //   return;
  // }
  config = readConfig();
  compileAll();
...
compileLess = function(location){
  var srcLess = location === undefined ? './res/less/main.less' : location + '/res/less/main.less',
      ...
      imports= location === undefined ? './res/less' : location+'/res/less',
